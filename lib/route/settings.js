...
router.post('/company/accountantdata/', function (req, res) {
  ...
  let start_date,
    ...
    csv = '',
    bank_holiday_map = {},
    summary_date = moment().subtract(1, 'month');
  ...
  req.user
    ...
    .then(function(company) {
      ...
      company.users.forEach(function(iterate_user) {
        ...
        if (!(iterate_user.my_leaves.length === 0)) {
          ...
          iterate_user.my_leaves.forEach(function(leave) {
            ...
            if (summary_date.isBetween(start_date, end_date, 'month', '[]')) {

              start_date_in_month = start_date.isSame(summary_date, 'month') ? start_date : summary_date.startOf('month');
              end_date_in_month = end_date.isSame(summary_date, 'month') ? end_date : summary_date.endOf('month');
              ...
              if (end_date === end_date_in_month && [2,3].includes(leave.day_part_end) && start_date !== end_date) {
                days_off -= 0.5;
              }
      ...
      res.attachment(
        'accountant_' + moment().subtract(1, 'month').format('YYYY-MM') + '.csv'
      );
