...
router.post('/company/accountantdata/', function (req, res) {
  ...
  let start_date,
    ...
    layout = {},
    bank_holiday_map = {},
    month = moment(req.param('date_format'));

  layout.name = month.format('MM-YY');
  layout.cols = [
    {
      caption: 'Employee',
      type: 'text'
  ...
  req.user
    ...
    .then(function(company) {
      ...
      company.leave_types.forEach(function (leave_type) {
        layout.cols.push(
          {
            caption: leave_type.name,
            type: 'number'
          }
        )
      });

      company.users.forEach(function(iterate_user) {

        row = [];
        row[0] = iterate_user.name + ' ' + iterate_user.lastname;
        
        for (let i = 1; i < layout.cols.length; i++) {
          row[i] = 0;
        }

        if (!(iterate_user.my_leaves.length === 0)) {

          iterate_user.my_leaves.forEach(function(leave) {

            if (month.isBetween(start_date, end_date, 'month', '[]') || leave.status !== 2) { // status 2 = approved

              start_date = moment(leave.date_start);
              end_date = moment(leave.date_end);

              start_date_in_month = start_date.isBefore(month.startOf('month')) ? month.startOf('month') : start_date;
              end_date_in_month = end_date.isAfter(month.endOf('month')) ? month.endOf('month') : end_date;

              // Remove half day absences
              if (start_date === start_date_in_month && [2,3].includes(leave.day_part_start)) {
                row[leave.id] -= 0.5;
              }

              if (end_date === end_date_in_month && [2,3].includes(leave.day_part_end) && ! start_date.isSame(end_date, 'day')) {
                row[leave.id] -= 0.5;
              }

              for (let days_iterator = start_date_in_month; days_iterator.isSameOrBefore(end_date_in_month); days_iterator.add(1, 'd')) {

                if(req.user.cached_schedule.is_it_working_day({ day: days_iterator}) && !bank_holiday_map[days_iterator.format('YYYY-MM-DD')]) {
                  row[leave.id] ++;
                }
              }
            }
      ...
      res.attachment(
        'accountant_' + month.format('YYYY-MM') + '.xlsx'
      );

      res.send(new Buffer(excel.execute(layout), 'binary'));
    });
});
