...
module.exports = function(sequelize, DataTypes) {
  var Company = sequelize.define("Company", {
    ...
    instanceMethods : {
      ...
      get_available_export_months() {

        var now = moment().endOf('month'),
            months_array = [];

        for (var month_iterator = moment(this.getDataValue('createdAt')).endOf('month'); month_iterator.isSameOrBefore(now); month_iterator.add(1, 'month')) {
          months_array.push(month_iterator.format('YYYY-MM'));
        }
        return months_array;
      },
